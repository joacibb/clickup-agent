...
export async function getRecentUpdates({ CLICKUP_TOKEN, USER_ID, TEAM_ID, HOURS_BACK = 2 } = {}) {
  ...
  // Fetch all non-closed tasks assigned to the user (no date filter, so old tasks are included)
  const tasksUrl = `https://api.clickup.com/api/v2/team/${TEAM_ID}/task?assignees[]=${USER_ID}&include_closed=false&subtasks=true`;
...
export async function getRecentTasks({ CLICKUP_TOKEN, USER_ID, TEAM_ID, DAYS_BACK = 30 } = {}) {
  if (!CLICKUP_TOKEN) throw new Error('Missing CLICKUP_TOKEN');
  if (!USER_ID) throw new Error('Missing USER_ID');
  if (!TEAM_ID) throw new Error('Missing TEAM_ID');

  const daysBack = Number(DAYS_BACK) || 7;
  const since = new Date(Date.now() - daysBack * 24 * 60 * 60 * 1000);
  const headers = { Authorization: CLICKUP_TOKEN };

  // Remove date filter for debugging — get all tasks
  const tasksUrl = `https://api.clickup.com/api/v2/team/${TEAM_ID}/task?assignees[]=${USER_ID}&include_closed=true&subtasks=true&page=0&limit=100`;
  console.log(`[getRecentTasks] Fetching tasks for user ${USER_ID} team ${TEAM_ID}`);
  const tasksRes = await fetch(tasksUrl, { headers });
  if (!tasksRes.ok) {
    const text = await tasksRes.text().catch(() => '');
    console.error(`[getRecentTasks] API error: ${tasksRes.status} ${text}`);
    throw new Error(`Failed fetching tasks: ${tasksRes.status} ${tasksRes.statusText} ${text}`);
  }
  const tasksData = await tasksRes.json();
  const tasks = tasksData.tasks || [];
  console.log(`[getRecentTasks] Got ${tasks.length} tasks from ClickUp`);
  if (tasks.length > 0) {
    console.log(`[getRecentTasks] First task keys:`, Object.keys(tasks[0]));
  } else {
    console.log(`[getRecentTasks] Raw response keys:`, Object.keys(tasksData));
  }
  ...
  return tasks.map(t => ({
    ...
    statusColor: t.status?.color || '#999',
    closed: t.closed || false,
    dateCreated: t.date_created ? parseDate(t.date_created)?.getTime() || null : null,
    dateUpdated: t.date_updated ? parseDate(t.date_updated)?.getTime() || null : null,
    dueDate: t.due_date ? parseDate(t.due_date)?.getTime() || null : null,
  }));
}
