...
export async function getRecentTasks({ CLICKUP_TOKEN, USER_ID, TEAM_ID, DAYS_BACK = 7 } = {}) {
  if (!CLICKUP_TOKEN) throw new Error('Missing CLICKUP_TOKEN');
  if (!USER_ID) throw new Error('Missing USER_ID');
  ...
  const tasksUrl = `https://api.clickup.com/api/v2/team/${TEAM_ID}/task?assignees[]=${USER_ID}&include_closed=true&subtasks=true&date_created_gt=${since.getTime()}&order_by=date_updated&reverse=true`;
  const tasksRes = await fetch(tasksUrl, { headers });
  if (!tasksRes.ok) {
    const text = await tasksRes.text().catch(() => '');
    throw new Error(`Failed fetching tasks: ${tasksRes.status} ${tasksRes.statusText} ${text}`);
